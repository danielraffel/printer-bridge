...
@Test
func bridgeStatusSupportsMultipleManagedPrintersWithDistinctProxyPorts() {
    let runner = InventoryStubCommandRunner { executable, arguments in
        let command = ([executable] + arguments).joined(separator: " ")
        ...
        case "/usr/bin/lpstat -p":
            return CommandResult(
                ...
                standardOutput: """
                printer Brother_HL_2170W_series is idle.  enabled since Wed Aug 13 12:11:45 2025
                printer Office_Printer is idle.  enabled since Wed Aug 13 12:11:45 2025
                """,
                standardError: ""
            )
        case "/usr/bin/lpstat -v":
            return CommandResult(
                ...
                exitCode: 0,
                standardOutput: """
                device for Brother_HL_2170W_series: usb://Brother/HL-2170W
                device for Office_Printer: socket://10.0.0.10
                """,
                standardError: ""
            )
        case "/usr/bin/lpstat -l -p Brother_HL_2170W_series":
        ...
        case "/usr/bin/lpstat -l -p Office_Printer":
            return CommandResult(
                executable: executable,
                arguments: arguments,
                exitCode: 0,
        ...
        case "/usr/bin/lpoptions -p Brother_HL_2170W_series -l",
             "/usr/bin/lpoptions -p Office_Printer -l":
            return CommandResult(
                executable: executable,
                arguments: arguments,
                exitCode: 0,
        ...
        case "/usr/bin/ipptool -tv ipp://localhost:631/printers/Office_Printer /usr/share/cups/ipptool/get-printer-attributes.test":
            return CommandResult(
                executable: executable,
                arguments: arguments,
                exitCode: 0,
                ...
                "/usr/share/cups/ipptool/get-printer-attributes.test":
                    Get-Printer-Attributes:
                        ...
                        printer-make-and-model (textWithoutLanguage) = Office Printer CUPS
                        document-format-supported (1setOf mimeMediaType) = application/pdf,image/pwg-raster,image/urf
                        printer-resolution-supported (1setOf resolution) = 600dpi
                """,
                standardError: ""
    ...
    let configuration = BridgeConfiguration(
        selectedQueueName: "Brother_HL_2170W_series",
        printers: [
            ManagedPrinterConfiguration(queueName: "Brother_HL_2170W_series", isEnabled: true, proxyPort: 8631),
            ManagedPrinterConfiguration(queueName: "Office_Printer", isEnabled: true, proxyPort: 8632),
        ],
        exposureMode: .proxy
    )
    ...
    #expect(snapshot.managedPrinters.count == 2)
    #expect(snapshot.enabledPrinterCount == 2)
    #expect(snapshot.livePrinterCount == 2)
    #expect(snapshot.publishableAdvertisements.map(\.port).sorted() == [8631, 8632])
    #expect(snapshot.status(forQueueNamed: "Office_Printer")?.advertisement?.printerURI == "ipp://test-host.local:8632/printers/Office_Printer")
}
